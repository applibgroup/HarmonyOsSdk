...
var forInStatement_1 = require("./statement/forInStatement");
...
var Compiler = /** @class */ (function () {
    ...
    Compiler.prototype.storeSpecialArg2LexEnv = function (arg) {
        ...
        if (v && v.isLexVar) {
            ...
            var env = pandaGen.GetCurLexEnv();
            var vreg = pandaGen.getVregForVariable(variableInfo.v);
            var slot = variableInfo.v.idxLex;
            pandaGen.storeLexicalVar(env, this.rootNode, variableInfo.level, slot, vreg);
        }
    };
    ...
    Compiler.prototype.compileFunctionParameterDeclaration = function (decl) {
        ...
        for (var index = 0; index < decl.parameters.length; ++index) {
            ...
            var paramRef = lreference_1.LReference.createLRef(this, parameter, true);
            var variable = void 0;
    ...
    Compiler.prototype.compileStatement = function (stmt) {
        ...
        switch (stmt.kind) {
            ...
            case ts.SyntaxKind.ForInStatement: //line 281
                forInStatement_1.compileForInStatement(stmt, this);
                break;
    ...
    Compiler.prototype.compileBlock = function (block) {
        ...
        hoisting_1.hoistFunctionInBlock(this.scope, this.pandaGen, strictMode_1.isStrictMode(block));
    ...
    Compiler.prototype.compileVariableDeclaration = function (decl, isExported) {
        ...
        var lref = lreference_1.LReference.createLRef(this, decl.name, true);
        if (decl.initializer) {
    ...
    Compiler.prototype.compileDoStatement = function (stmt) {
        this.pushScope(stmt);
        ...
        this.pandaGen.label(stmt, loopStartLabel);
        this.compileStatement(stmt.statement);
        this.pandaGen.label(stmt, conditionLabel);
        this.compileCondition(stmt.expression, loopEndLabel);
        this.pandaGen.branch(stmt, loopStartLabel);
        this.pandaGen.label(stmt, loopEndLabel);
        labelTarget_1.LabelTarget.popLabelTarget();
        this.popScope();
    };
    Compiler.prototype.compileWhileStatement = function (stmt) {
        this.pushScope(stmt);
        var loopStartLabel = new irnodes_1.Label();
        var loopEndLabel = new irnodes_1.Label();
    ...
    Compiler.prototype.compileForStatement = function (stmt) {
        var _this = this;
        this.pushScope(stmt);
        // compile the intitializer
        if (stmt.initializer) {
            if (stmt.initializer.kind == ts.SyntaxKind.VariableDeclarationList) {
                var declList = stmt.initializer;
                declList.declarations.forEach(function (decl) { return _this.compileVariableDeclaration(decl); });
            }
            else {
        ...
        this.pandaGen.label(stmt, loopStartLabel);
        if (stmt.condition) {
            this.compileCondition(stmt.condition, loopEndLabel);
        }
        // loopBody
        this.compileStatement(stmt.statement);
        // loopIncrementor
        this.pandaGen.label(stmt, incLabel);
        if (stmt.incrementor) {
            this.compileExpression(stmt.incrementor);
        }
        this.pandaGen.branch(stmt, loopStartLabel);
        this.pandaGen.label(stmt, loopEndLabel);
        labelTarget_1.LabelTarget.popLabelTarget();
    ...
    Compiler.prototype.compileContinueStatement = function (stmt) {
        ...
        this.compileFinallyBlockBeforeControlFlowChange(continueLabelTarget.getTryStatement(), ControlFlowChange.Continue, continueLabelTarget.getContinueTargetLabel());
    ...
    Compiler.prototype.compileBreakStatement = function (stmt) {
        ...
        this.compileFinallyBlockBeforeControlFlowChange(breakLabelTarget.getTryStatement(), ControlFlowChange.Break, undefined);
    ...
    Compiler.prototype.compileFinallyBlockBeforeControlFlowChange = function (endTry, cfc, continueTargetLabel) {
        ...
        if (!decl.name) {
            ...
            if (hasExport && hasDefault) {
                if (this.scope instanceof scope_1.ModuleScope) {
                    var internalName = this.compilerDriver.getFuncInternalName(decl);
                    this.pandaGen.defineFunction(debuginfo_1.NodeKind.FirstNodeOfFunction, decl, internalName);
    ...
    Compiler.prototype.compileCondition = function (expr, ifFalseLabel) {
        ...
        if (expr.kind == ts.SyntaxKind.BinaryExpression) {
            ...
            switch (binExpr.operatorToken.kind) {
                ...
                case ts.SyntaxKind.AmpersandAmpersandToken: {
                    ...
                    pandaGen.condition(binExpr, ts.SyntaxKind.EqualsEqualsToken, vregisterCache_1.getVregisterCache(pandaGen, vregisterCache_1.CacheList.True), ifFalseLabel);
                    this.compileExpression(binExpr.right);
                ...
                case ts.SyntaxKind.BarBarToken: {
                    ...
                    this.compileExpression(binExpr.left);
                    pandaGen.toBoolean(binExpr.left);
                    pandaGen.condition(binExpr, ts.SyntaxKind.EqualsEqualsToken, vregisterCache_1.getVregisterCache(pandaGen, vregisterCache_1.CacheList.False), endLabel);
                    this.compileExpression(binExpr.right);
                    pandaGen.toBoolean(binExpr.right);
                    pandaGen.condition(binExpr, ts.SyntaxKind.ExclamationEqualsToken, vregisterCache_1.getVregisterCache(pandaGen, vregisterCache_1.CacheList.False), ifFalseLabel);
        ...
        pandaGen.condition(expr, ts.SyntaxKind.EqualsEqualsToken, vregisterCache_1.getVregisterCache(pandaGen, vregisterCache_1.CacheList.True), ifFalseLabel);
    };
    ...
    Compiler.prototype.compileRegularExpressionLiteral = function (regexp) {
        var pandaGen = this.pandaGen;
        var ctorReg = pandaGen.getTemp();
        var newTargetReg = pandaGen.getTemp();
        var regexpText = regexp.text;
        var regexpPatternReg = pandaGen.getTemp();
        ...
        var argsArray = [ctorReg, newTargetReg, regexpPatternReg];
        if (regexpFlags.length > 0) {
            var regexpFlagsReg = pandaGen.getTemp();
        ...
        pandaGen.freeTemps.apply(pandaGen, argsArray);
    };
    Compiler.prototype.compileIdentifier = function (id) {
        ...
        if (!v) {
            ...
            this.emitLoad(id, { scope: scope, level: level, v: v });
        }
    };
    ...
    Compiler.prototype.compileFunctionExpression = function (expr) {
        var internalName = this.compilerDriver.getFuncInternalName(expr);
    ...
    Compiler.prototype.compileDeleteExpression = function (expr) {
        ...
        switch (unaryExpr.kind) {
            case ts.SyntaxKind.Identifier:
                ...
                var _a = this.scope.find(name_2), scope = _a.scope, level = _a.level, v = _a.v;
            ...
            case ts.SyntaxKind.ElementAccessExpression:
                ...
                var _b = memberAccessExpression_1.getObjAndProp(unaryExpr, objReg, propReg, this), obj = _b.obj, prop = _b.prop;
                switch (typeof prop) {
                    case "string":
    ...
    Compiler.prototype.compilePrefixUnaryExpression = function (expr) {
        ...
        switch (expr.operator) {
            ...
            case ts.SyntaxKind.MinusMinusToken: // line 74
                var lref = lreference_1.LReference.createLRef(this, expr.operand, false);
                lref.getValue();
    ...
    Compiler.prototype.compileLogicalExpression = function (expr) {
        ...
        switch (expr.operatorToken.kind) {
            case ts.SyntaxKind.AmpersandAmpersandToken: { // line 83
                ...
                pandaGen.condition(expr, ts.SyntaxKind.EqualsEqualsToken, vregisterCache_1.getVregisterCache(pandaGen, vregisterCache_1.CacheList.True), leftFalseLabel);
            ...
            case ts.SyntaxKind.BarBarToken: { // line 84
                ...
                pandaGen.storeAccumulator(expr, lhs);
                pandaGen.toBoolean(expr.left);
                pandaGen.condition(expr, ts.SyntaxKind.EqualsEqualsToken, vregisterCache_1.getVregisterCache(pandaGen, vregisterCache_1.CacheList.False), leftTrueLabel);
    ...
    Compiler.prototype.compileAssignmentExpression = function (lhs, rhs, operator) {
        var lref = lreference_1.LReference.createLRef(this, lhs, false);
    ...
    Compiler.prototype.pushScope = function (node) {
        // let scope = <Scope>this.compilerDriver.getScopeOfNode(node);
    ...
    Compiler.prototype.getThis = function (node, res) {
        ...
        if (thisInfo.v.isLexVar) {
            ...
            pandaGen.loadLexicalVar(env, node, thisInfo.level, slot);
    ...
    Compiler.prototype.setThis = function (node) {
        ...
        if (thisInfo.v.isLexVar) {
            ...
            pandaGen.storeLexicalVar(env, node, thisInfo.level, slot, value);
    ...
    Compiler.prototype.emitStore = function (node, variable, isDeclaration) {
        if (variable.v instanceof variable_1.LocalVariable) {
            ...
            if (variable.scope && variable.level > 0) {
            ...
            throw new Error("unvalid lhsRef to store");
        }
    };
    Compiler.prototype.emitLoad = function (node, variable) {
