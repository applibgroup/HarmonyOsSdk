...
function hasDuplicateEntryInScope(decl1, decl2, scope) {
    ...
    if (scope instanceof scope_1.LocalScope) {
        if (strictMode_1.isStrictMode(decl1.node)) {
            ...
            // In non-strict mode, eg2. is legal;
        }
    }
    return false;
}
...
function checkMetaProperty(node) {
    ...
    switch (node.keywordToken) {
        ...
        case ts.SyntaxKind.ImportKeyword:
            if (text != "meta") {
                var args_1 = [text, jshelpers_1["default"].tokenToString(node.keywordToken), "meta"];
                throw new diagnostic_1.DiagnosticError(node, diagnostic_1.DiagnosticCode._0_is_not_a_valid_meta_property_for_keyword_1_Did_you_mean_2, file, args_1);
            }
            break;
    }
}
...
function checkModifiers(node) {
    ...
    if (ts.isConstructorDeclaration(node)) {
        ...
        if (flags & ts.ModifierFlags.Abstract) {
            throw new diagnostic_1.DiagnosticError(lastStatic, diagnostic_1.DiagnosticCode._0_modifier_cannot_appear_on_a_constructor_declaration, file, ["abstract"]); // TODO: GH#18217
        }
        else if (flags & ts.ModifierFlags.Async) {
