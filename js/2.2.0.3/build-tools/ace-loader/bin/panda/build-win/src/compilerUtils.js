...
exports.emitDestructuring = void 0;
...
function emitDestructuring(pattern, pandaGen, compiler) {
    var rhs = pandaGen.getTemp();
    pandaGen.storeAccumulator(pattern, rhs);
    if (util_1.isArrayBindingOrAssignmentPattern(pattern)) {
        emitArrayDestructuring(pattern, pandaGen, compiler);
    }
    if (util_1.isObjectBindingOrAssignmentPattern(pattern)) {
        emitObjectDestructuring(pattern, pandaGen, compiler);
    }
    pandaGen.loadAccumulator(pattern, rhs);
    pandaGen.freeTemps(rhs);
}
exports.emitDestructuring = emitDestructuring;
function emitArrayDestructuring(arr, pandaGen, compiler) {
    ...
    for (var i = 0; i < arr.elements.length; i++) {
        ...
        if (!isDeclaration && ts.isBinaryExpression(element)) {
            // TODO it should be removed in the end;
        ...
        var lRef = lreference_1.LReference.createLRef(compiler, target, isDeclaration ? true : false);
...
function emitRestElement(restElement, iterator, iterResult, pandaGen, compiler, isDeclaration) {
    ...
    var lRef = lreference_1.LReference.createLRef(compiler, target, isDeclaration);
...
function emitObjectDestructuring(obj, pandaGen, compiler) {
...
function emitRestProperty(restProperty, excludedProp, obj, pandaGen, compiler) {
    ...
    var lRef = lreference_1.LReference.createLRef(compiler, target, true);
