...
/**
 * Contains variety of system and app settings ,provides functions for reading and writing these system settings
 * and provides methods for querying the capabilities of applications.
...
declare namespace settings {
   ...
   * @since 7
   */
  function getValue(name: string, callback: AsyncCallback<string>): void;
  function getValue(name: string): Promise<string>;
   ...
   * @hide SystemApi
   */
  function setValue(name: string, value: string, callback: AsyncCallback<void>): void;
  function setValue(name: string, value: string): Promise<void>;

  /**
   * Enables or disables airplane mode.
   *
   * <p>After airplane mode is set, the common event
   * {@link ohos.event.commonevent.CommonEventSupport#COMMON_EVENT_AIRPLANE_MODE_CHANGED} is published.
   * <p><b>Permissions:</b> {@link ohos.security.SystemPermission#MANAGE_SECURE_SETTINGS}
   *
   ...
   *
   * <p>To draw over other applications, an application must declare the {@code ohos.permission.SYSTEM_FLOAT_
   * WINDOW} permission and get an explicit approval from end users. To achieve so, the application must display a
   * dialog box to ask end users for the permission.
   *
   * @param context Indicates the application context.
